...
class ApiProductos with ChangeNotifier {
  ...
  Future<void> getProductos(BuildContext context, String token) async {
    ...
    final response = await http.get(Uri.parse(UrlApis.URL_API + 'index.php/stock'), headers: {"Authorization": "Digest $token}"});
    if (response.statusCode == 200) {
      ProductosJson prd = ProductosJson.fromJson(jsonDecode(response.body));

      for (int i = 0; i < prd.data.length; i++) {
        //print("$i de ${prd.data.length} Se agregara: ${prd.data[i].itemName}");
        providerSQliteProducto.nuevoProducto(Producto(nombre: Utf8Decoder().convert(prd.data[i].itemName.codeUnits), varianteId: prd.data[i].variantId, enStock: prd.data[i].inStock.toString()));
      }
